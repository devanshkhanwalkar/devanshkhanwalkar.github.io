$(function(){init(12)});var upperBound=4096;var power=12;function init(u){upperBound=1;if($("#pow").val()){if($("#pow").val()<=2){window.alert("Enter n greater than 2");return false}else{u=$("#pow").val();power=u}$("#pow").blur()}else power=u;for(var b=u;b>=1;b--)upperBound=upperBound*2;changeGame();$("#title").text(upperBound);$("#myTable").hide();$("#score").val(score);$("#moves").val(moves);setTimeout("gameloading()",100)}
function restart(){moves=0;score=0;for(var m=0;m<=3;m++)for(var n=0;n<=3;n++)$("#"+m+""+n).val("");init(power)}function changeGame(){score=0;moves=0;for(var m=0;m<=3;m++)for(var n=0;n<=3;n++)$("#"+m+""+n).val("")}function gameloading(){$("#myTable").show();changeColor();var r1=Math.floor(Math.random()*3+1);var r2=Math.floor(Math.random()*3+1);for(var i=0;i<r1;i++)for(var j=0;j<r2;j++){var k=Math.floor(Math.random()*2+0);if(k==1)$("#"+i+""+j).val("2")}changeColor()}var count;var moves=0;
var score=0;var randomElement=0;document.onkeydown=checkKey;function checkKey(e){e=e||window.event;if(e.keyCode=="37"){left();moves++}else if(e.keyCode=="38"){up();moves++}else if(e.keyCode=="39"){right();moves++}else if(e.keyCode=="40"){down();moves++}gameOver();$("#score").val(score);$("#moves").val(moves)}function left(){for(var i=0;i<=3;i++){changeColor();subleft(i);changeColor()}addLeftMove();changeColor()}
